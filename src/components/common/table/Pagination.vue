<template>
  <div class="pagination">
    ...
    <button :disabled="!enabledPrevGroup" @click='onClickPrevGroup'>prev group</button>
    <button :disabled="!enabledPrevPage" @click='onClickPrevPage'>prev page</button>
    <button v-for="n in showPageCount" :key='n'
      :class="{ current_page: selectedPageIndex === startPageIndex + n - 1 }"
      @click="() => onClickNum( startPageIndex + n - 1 )">
      {{ startPageIndex + n }}
    </button>
    <button :disabled="!enabledNextPage" @click='onClickNextPage'>next page</button>
    <button :disabled="!enabledNextGroup" @click='onClickNextGroup'>next group</button>
